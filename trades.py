...
class Trade():
    ...
    def new_trade(self, trade_id: str, order_type: str, side: str, long_or_short: str, enter_or_exit: str, price: float = 0.00, stop_limit_price: float = 0.00) -> dict:

        self.trad_id = trade_id
    ...
    def good_til_cancel(self, cancel_time: datetime) -> None:

        self.order['duration'] = 'GOOD_TIL_CANCEL'
    ...
    def modify_side(self, side: Optional[str], order_leg_id: int = 0) -> None:
        ...
        if side and side not in ['buy', 'sell', 'sell_short', 'buy_to_cover']:
            raise ValueError("You passed throuhg an invalid sie")
    ...
    def number_of_legs(self) -> int:
        ...
        return len(self.order['orderLegColeltion'])