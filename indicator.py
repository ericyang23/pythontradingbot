...
class Indicator():
    ...
    def set_indicator_signals(self, indicator: str, buy: float, sell: float, condiction_buy: Any, condition_sell: Any) -> None:

        # If there is no signal for the indicator set a template
        ...
        self._indicator_signals[indicator]['buy_operator'] = condiction_buy
    ...
    def rsi(self, period: int, method: str = 'wilders') -> pd.DataFrame:
        ...
        self._frame.drop(
            labels=['ewma_up', 'ewmna_down', 'down_day', 'up_day' 'change_in_price'],
            axis=1,
            inplace=True
        )
    ...
    def refresh(self):
        ...
        # Loop throuhg all the stored indicators
